...
| 2010 | `CLIENT_CLOSED` | `ClientClosed`, `ConnectionAborted` | `client` | `ClientClosed`: the client was already closed. `ConnectionAborted` (message `Connection aborted: …`): `connect()` given up because `disconnect()` was called before the connection was established (Rust async client, Node, Python, and the C#, Go, Java and C++ bindings, which report it as the `ClientClosed` variant). Rust async client: also a `connect()` or `wait_connected()` waiting on an automatic reconnect when `disconnect()` or `force_close()` is called (`ConnectionAborted`), or when the client closed without the reconnect giving up (`ClientClosed`). Python and the C#, Go, Java and C++ bindings: also a `connect()` waiting on an automatic reconnect when `disconnect()` is called. |
| 2011 | `ALREADY_CONNECTED` | `AlreadyConnected` | `client` | WebSocket `connect()` called while connected or while another `connect()` is in progress. During an automatic reconnect `connect()` waits for the reconnect instead (it resolves when the reconnect succeeds, or fails with 2010, 3005 or 2002) in the Rust async client, Python and the C#, Go, Java and C++ bindings; Python and those bindings count the connection as connected once `connect()` has returned or its `authenticated` event has been delivered, so a `connect()` from a `disconnect` handler waits even if core has already reconnected. Node and the Rust sync client still return 2011 there. |
...
| 3005 | `RECONNECT_FAILED` | `ReconnectFailed` | `network` | Automatic reconnection gave up: after its last attempt, or because an attempt's credentials were rejected (an `unauthenticated` event precedes it) (Node / Python `error`; C#, Go, Java and C++ have a dedicated reconnect-failed callback). Rust async client: a `connect()` or `wait_connected()` waiting on the reconnect fails with `ReconnectFailed { attempts }` after the last attempt; after a rejection it fails with `AuthError` (2002). Python and the C#, Go, Java and C++ bindings: a `connect()` waiting on the reconnect fails likewise (Python `WebSocketError`, the bindings' `WebSocketError` variant, both with code 3005). |
