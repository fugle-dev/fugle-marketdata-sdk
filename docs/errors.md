...
| 2010 | `CLIENT_CLOSED` | `ClientClosed`, `ConnectionAborted` | `client` | `ClientClosed`: the client was already closed. `ConnectionAborted` (message `Connection aborted: …`): `connect()` given up because `disconnect()` was called before the connection was established, or a `connect()` waiting on an automatic reconnect (see 2011) when `disconnect()` is called — in the Rust async client (also `wait_connected()`, and `force_close()`), Node, Python, and the C#, Go, Java and C++ bindings, which report it as the `ClientClosed` variant. Rust async client: a waiting `connect()` / `wait_connected()` also gets `ClientClosed` when the client closed without the reconnect giving up. Node: a waiting `connect()` also gets 2010 when the connection ends without reconnecting. |
| 2011 | `ALREADY_CONNECTED` | `AlreadyConnected` | `client` | WebSocket `connect()` called while connected or while another `connect()` is in progress. During an automatic reconnect `connect()` waits for the reconnect instead (#230), in the Rust async client and in every language (Node, Python, C#, Go, Java, C++): it resolves once the reconnect succeeds and the subscriptions have been re-sent (Node: with the reconnect's `authenticated` data), or fails with 2010, 3005 or 2002 (Node: the server's `data` object). Node counts the connection as connected once its `authenticated` event has been delivered to the listeners; Python and the C#, Go, Java and C++ bindings once `connect()` has returned or that event has been delivered. So a `connect()` from a `disconnect` handler waits even if core has already reconnected. Only the Rust sync client still returns 2011 during a reconnect. The message (`Already connected; connect() is not needed while the connection is open or being opened`) does not suggest `disconnect()` first, because after an automatic reconnect, calling `disconnect()` closes the connection it restored (see 3006). |
...
| 3005 | `RECONNECT_FAILED` | `ReconnectFailed` | `network` | Automatic reconnection gave up: after its last attempt, or because an attempt's credentials were rejected (an `unauthenticated` event precedes it) (Node / Python `error`; C#, Go, Java and C++ have a dedicated reconnect-failed callback). A `connect()` waiting on the reconnect (see 2011) fails with it after the last attempt — Rust async client `ReconnectFailed { attempts }` (also `wait_connected()`), Node an `Error` with `code` 3005, Python `WebSocketError`, the C#, Go, Java and C++ bindings their `WebSocketError` variant; after a rejection it fails with `AuthError` (2002) instead (Node: rejects with the server's `data` object). |
| 3006 | `RECONNECT_CONFLICT` | — | `client` | A warning, not a failure: `connect()` was called less than 30 seconds after `disconnect()` or `force_close()` closed a connection that automatic reconnect had restored less than 30 seconds before — what code that also reconnects on its own does, which loops (#226, #242, [migration guide §5](../MIGRATION.md#5-auto-reconnect-is-on-by-default)). A close with no `connect()` after it, and a `connect()` that joins a reconnect under way, are not warned about. Reported once per client, by that `connect()` before its connecting event; the `connect()` goes ahead. In Rust a closed client cannot connect again: code that builds a new client for each connection shares `ReconnectConflictHandle` between them (`use_reconnect_conflict_handle`), as the bindings do. C#, Go, Java and C++ report it through their error callback (the C# legacy-compatible `FugleWebsocketClient` raises it as `OnException`), Rust as `ConnectionEvent::Error`. Python issues it as a `RuntimeWarning` and Node as a process warning (`FugleReconnectWarning`, code `FUGLE_RECONNECT_CONFLICT`) instead of an `error` event. |
