...
| 3006 | `RECONNECT_CONFLICT` | — | `client` | A warning, not a failure: `disconnect()` or `force_close()` closed a connection that automatic reconnect had restored less than 30 seconds earlier — what code that also reconnects on its own does, which loops (#226, [migration guide §5](../MIGRATION.md#5-auto-reconnect-is-on-by-default)). Reported once per client, right before that close's disconnect event; the close goes ahead. C#, Go, Java and C++ report it through their error callback (the C# legacy-compatible `FugleWebsocketClient` raises it as `OnException`), Rust as `ConnectionEvent::Error`. Python issues it as a `RuntimeWarning` and Node as a process warning (`FugleReconnectWarning`, code `FUGLE_RECONNECT_CONFLICT`) instead of an `error` event. |
