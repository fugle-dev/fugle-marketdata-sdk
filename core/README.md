...
match client.stock().intraday().quote().symbol("2330").send() {
    ...
    Err(e @ MarketDataError::ApiError { .. }) => {
        // `info()` has the fields every binding exposes: code, source_kind,
        // message, status, body, request_id, headers.
        let info = e.info();
        eprintln!("API error {:?}: {:?} (headers: {:?})", info.status, info.body, info.headers);
    }
...
binding exposes under the same names; codes are in `error_code`. See the
[error reference](https://github.com/fugle-dev/fugle-marketdata-sdk/blob/main/docs/errors.md) for the field table, `source_kind` values and all
error codes.
