...
mod tests {
    ...
    #[test]
    fn test_institutional_trades_url_sends_sort() {
        let client = RestClient::new(Auth::SdkToken("test".to_string()));
        let base = client.get_base_url().to_string();
        for sort in ["asc", "desc"] {
            let url = InstitutionalTradesRequestBuilder::new(&client).symbol("2330").sort(sort).url().unwrap();
            assert_eq!(url, format!("{base}/stock/ownership/institutional-trades/2330?sort={sort}"));
        }
    }
}
